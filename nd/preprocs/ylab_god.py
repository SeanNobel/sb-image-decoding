...
from nd.utils.brain_preproc import brain_preproc, baseline_correction
from nd.utils.layout import load_god_montage
...
def get_segmented_ecog(args, ecog: h5py._hl.files.File) -> Tuple[np.ndarray, List[int]]:
    ...
    for i, (onset, offset) in enumerate(zip(onsets, offsets)):
        if onset == -1:  # or offset == -1:
            cprint(
                "Segment dropped: onset was NaN", "yellow"
            )  # or offset was NaN", "yellow")
        ...
        if args.baseline_ratio < 0:
            baseline.append(
                signals[
                    ...
                    onset
                    - int(
                        orig_sfreq * (-args.baseline_ratio) * args.max_seq_len
                    ) : onset,
                ]
            )
    ...
    if len(baseline) > 0:
        ...
        X = baseline_correction(
            X,
            baseline_len_samp=int(
                args.max_seq_len * args.baseline_ratio * args.brain_resample_sfreq
            ),
        )
...
def rearrange_montage(
    montage: np.ndarray, chnames_montage: List[str], ecog: h5py._hl.files.File
) -> Tuple[List]:
    """
    ...
    chnames_ecog = [
        "".join(
            [chr(c) for c in ecog[ecog["data_st"]["channel_names"][i, 0]][()].squeeze()]
        )
        for i in range(len(ecog["data_st"]["channel_names"]))
    ]
...
def preproc(
    ...
    try:
        montage, ch_names = load_god_montage(
            subject_name, args.freesurfer_dir, return_chnames=True
        )
    except FileNotFoundError:
        cprint(
            f"Subject {subject_name} was dropped, as its montage was not found.",
            "yellow",
        )
        return None
    ...
    if len(set(ch_names)) != len(ch_names):
        cprint(
            f"Subject {subject_name} was dropped, as its montage had duplicated channels.",
            "yellow",
        )
        return None
    ...
    if len(montage) < 10:
        cprint(
            f"Subject {subject_name} was dropped, as the number of channels found was less than 10. ({len(montage)})",
            "yellow",
        )
        return None
...
def main(args: DictConfig) -> None:
    ...
    for ecog_dir in glob.glob(f"{args.data_root}continuous_10k/*/"):
        ...
        data_dir = os.path.join(
            args.root_dir, "data/preprocessed/ylab/god", args.preproc_name, subject_name
        )
        ...
        cprint(
            f"Processing subject {subject_name}. ({len(fnames_trn)} train sessions, {len(fnames_val)} validation sessions)",
            "cyan",
        )
        ...
        for fname in fnames:
            ...
            np.savetxt(
                os.path.join(data_dir, f"image_{postfix}.txt"),
                Y,
                delimiter=",",
                fmt="%s",
            )
        ...
        assert (
            np.diff(np.stack(montages), axis=0).sum() == 0
        ), "Montage inconsistency detected."
