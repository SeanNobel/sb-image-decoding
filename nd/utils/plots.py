...
def plot_2d_latents_with_sorted_categories(
    ...
    ax[0].scatter(
        *train_Y.T, c=sort_categories(train_categories), s=1, cmap="gist_rainbow"
    )
    ax[0].set_title("Image train")
    ax[1].scatter(
        *test_Y.T, c=sort_categories(test_categories), s=1, cmap="gist_rainbow"
    )
    ax[1].set_title("Image test")
    ax[2].scatter(
        *train_Z.T, c=sort_categories(train_categories), s=1, cmap="gist_rainbow"
    )
    ax[2].set_title("Text (MEG) train")
    ax[3].scatter(
        *test_Z.T, c=sort_categories(test_categories), s=1, cmap="gist_rainbow"
    )
